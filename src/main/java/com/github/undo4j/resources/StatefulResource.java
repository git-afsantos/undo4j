...
public abstract class StatefulResource<T> extends AbstractResource<T> {
	 ...
	 * Checkpoints may be set directly from the committed state, or by reading
	 * again the internal resource, since the commit operation modifies it.
	 */
	...
	public StatefulResource(InternalResource<T> resource, LockManager lockManager) {
		this(resource, lockManager, false);
	}

	/** Parameter constructor of objects of class StatefulResource. */
	public StatefulResource(InternalResource<T> resource, LockManager lockManager, boolean buildEachUpdate) {
	...
	/**
	 * This method assumes the resource has been acquired. Chapter 17 of The
	 * Java Language Specification: "Writes and reads of volatile fields have
	 ...
	 * granted that no writers are able to modify the object referenced by the
	 * checkpoint.
	 */
	@Override
	...
	/**
	 * Subclasses should override this method to ensure that only one thread is
	 * applying a state at any time.
	 */
	...
	public boolean equals(Object o) {
		...
		return (getInternalResource().equals(n.getInternalResource()) && this.checkpoint.equals(n
