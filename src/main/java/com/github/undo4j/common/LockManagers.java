...
public final class LockManagers {
	...
	/** */
	public static LockManager newLockManager(IsolationLevel isolation, AcquireStrategy strategy) {
	...
	/** */
	public static LockManager newTimedLockManager(IsolationLevel isolation, long time, TimeUnit timeUnit) {
		Check.checkArgument(isolation);
	...
	/** */
	private static LockManager newIsolatedLockManager(IsolationLevel isolation, LockStrategy strategy) {
		switch (isolation) {
		case EXCLUSIVE:
			return new ExclusiveLockManager(strategy);

		case READ_WRITE:
			return new ReadWriteLockManager(strategy);

		case READ_COMMITTED:
			return new ShareableLockManager(strategy);
